...
<div
	...
	class=" w-full {className} rounded-xl flex relative group hover:bg-gray-100 dark:hover:bg-white/[0.08] {$page
		.url.pathname === `/channels/${channel.id}`
		? 'bg-gray-100 dark:bg-white/[0.045] selected'
...
>
	...
	{#if ['dm'].includes(channel?.type)}
		...
		>
			<button
				type="button"
				class="p-0.5 dark:hover:bg-white/[0.08] rounded-lg touch-auto"
				on:click={async (e) => {
